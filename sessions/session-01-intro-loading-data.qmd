---
title: "session 01: working with raster data"
subtitle: "Getting started with spatial data and rasterio"
...
::: {style="margin-top: 30px; margin-bottom: 45px; text-align: center;"}
![](../images/session_01_affine.webp){fig-alt="illustration of how the affine transform maps a pixel's row/column index to a real-world x/y coordinate, ideally with a small grid and an arrow pointing to a coordinate pair" width="100%"}
:::
...
::: {style="text-align: justify;"}
Before drawing two spatial datasets together, we must confirm they share the same coordinate reference system. If they do not, the two layers are measured in different units or origins, and drawing them on the same axes places them in completely different, unrelated positions, which is exactly what causes the boundary to appear missing or wildly out of place. We cover CRS properly in [**Session 03**](session-03-raster-metadata-crs); for now we only need `.to_crs()` to fix a mismatch when we find one.
:::
